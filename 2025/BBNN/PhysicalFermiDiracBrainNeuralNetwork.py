...
class PhysicalFermiDiracBrainNeuralNetwork:
    ...
    def fermi_dirac_distribution(self, x):
        return 1 / (np.exp((x - np.mean(x)) / (self.temperature * self.boltzmann_constant)) + 1)
    
    def sample_activation(self, x):
        prob = self.fermi_dirac_distribution(x)
...
bbnn = PhysicalFermiDiracBrainNeuralNetwork(input_size=3, hidden_size=5, output_size=2, forehead_temperature=310.15, natural_unit=1.0, boltzmann_constant=1.38e-23)
